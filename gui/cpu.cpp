...
void CPU::run_one_cycle()
{
	...
	FTemp = FetchStage.run(stall, Decode_buffer[18], DecodeStage.pc, Decode_buffer[2], Decode_buffer[14], Decode_buffer[15]);
	...
	{
		Fetch_buffer[0] = -1;
		Decode_buffer[2] = DTemp[2];
		Decode_buffer[10] = DTemp[10];
		return;
	}
